...
def main():
    ...
    market = 'Bergen'
    train_network(market)


def train_network(market):
    # december a november
    # std v januari a porovnat
    global data
    print(data[market].shape)
    n = data[market].shape[0]
    #p = data[market].shape[1]
    print(n)
       ...
       # y_train = y_train[shuffle_indices]
        ...
        for i in range(0, len(X_train) // batch_size):
           ...
           #  batch_y = y_train[start:start + batch_size]
            ...
            net.run(opt, feed_dict={X: batch_x})

            # Show progress
            if np.mod(i, 50) == 0:
                # MSE train and test
                mse_train.append(net.run(mse, feed_dict={X: X_train}))
                mse_test.append(net.run(mse, feed_dict={X: X_test}))
