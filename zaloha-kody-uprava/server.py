...
def upload_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename) :
            ...
            file.save('/var/www/ostrodickyApp/ostrodickyApp/static/uploads/'+g_$
            data_csv = pd.read_csv('/var/www/ostrodickyApp/ostrodickyApp/static$
            ...
            return render_template('upload.html', info=info_data, labels=co$
            else:
            ...
            def choose_label():
                if request.method == 'POST':
                    ...
                    return render_template('upload.html', info=info_data, statistics=fan$
                ...
                def predict():
                    if request.method == 'POST':
							...
							model_fit,mape_statistical,result_arima = prediction_arima_flask(data[m$
							...
							model_neural,mape_network,result_network = prediction_neural_network_fl$
							mape_network = np.round(mape_network, 2)
							output = np.round(result_network, 2)
							result_network = [datumy, output]

							return render_template('prediction.html', mape_statistical = mape_stati$
											mape_network = mape_network, result_network = result_ne$
